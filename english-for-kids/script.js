...
function selectMode() {
    ...
    function startGame() {
        const cardsArr = createWordsSequence();
        const soundsArr = createWordsSounds();
        const failuresArr = [];
        const frontCards = document.querySelectorAll(".front-card");


        function createWordsSequence() {
            const cardsArr = cards[cards[0].indexOf(card.dataset.category) + 1];
            cardsArr.sort(() => Math.random() - 0.5);
            console.log(cardsArr);
            return cardsArr;
        }

        function createWordsSounds() {
            const soundsArr = [];
            for (let i = 0; i < cardsArr.length; i++) {
                soundsArr.push(cardsArr[i].audioSrc);
            }
        ...
        function checkAnswers() {
            if (this.closest(".card").dataset.word === cardsArr[0].word) {
                this.classList.add("right");
                cardsArr.shift();
                soundsArr.shift();
                playAudio();
            } else if (this.closest(".card").dataset.word !== cardsArr[0].word && !this.classList.contains("right")) {
                failuresArr.push(this.closest(".card").dataset.word);
                console.log(failuresArr)
            }
